...
//        factory.setAttribute("indent-number", 4)
    ...
    private fun convert(doc: Document) {
        ...
        doc.getElementsByTagNameNS(null, "path").iterable.map { it as Element }.forEach {
            ...
            val fillColorName = it.attributes.get(ANDROID_NS, "fillColor")
            if (fillColorName?.startsWith("@drawable/") == true) {
                val fileName = fillColorName.split("/").last()
                val file = File("./${fileName}.xml")
                if (file.readText().contains("<gradient")) {
                    val otherDoc = builder.parse(file.inputStream())
                    it.removeAttributeNS(ANDROID_NS, "fillColor")
                    val attrElement = doc.createElement("aapt:attr")
                    attrElement.setAttribute("name", "android:fillColor")
                    val importNode = doc.importNode(otherDoc.documentElement, true)
                    attrElement.appendChild(importNode)
                    it.appendChild(attrElement)
                }
            }
            it.fix()
        }
    ...
    private fun createLinearGradient(doc: Document, gradientElement: Element, id: String): Element {
        ...
        val tileMode = gradientElement.getAttributeNS(ANDROID_NS, "tileMode").lowercase()
        when (tileMode) {
            "clamp" ->
    ...
    private fun parseColor(color: String): Pair<String, Float?> {
        ...
        if (color.startsWith("@") || color.startsWith("?attr/")) {
            val name = color.split("/").last()
            val resourceHex = colors.getValue(name)
                ?: return Pair("#000000", null) //throw IllegalArgumentException("Color $name does not exists")
